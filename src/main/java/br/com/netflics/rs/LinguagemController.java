...
@RestController
@RequestMapping("/rs/crud/linguagems")
...
public class LinguagemController {
	...
	public static final Logger LOGGER = LoggerFactory.getLogger(LinguagemController.class);
	...
	public ResponseEntity get(@RequestParam MultiValueMap<String, String> mapParams) {
		...
		try {
			SearchParameters<FilterLinguagem> paginationParams = new SearchParameters<FilterLinguagem>(mapParams, FilterLinguagem.class);

			linguagems = linguagemService.get(paginationParams, context.getTenant());
			
			JsonPaginator<JsonLinguagem> paginator = JsonPaginator.of(Parser.toListJsonLinguagems(linguagems.getItems()),
				 linguagems.getActualPage(), linguagems.getPageSize(), linguagems.getOrder(), linguagems.getOrderBy(),linguagems.getTotalRecords());

			response = ResponseEntity.ok(paginator);

		} catch (Exception e) {
			String message = String.format("Não foi possivel carregar linguagems para os parametros %s [%s]", mapParams.toString(), e.getMessage());
	...
	public ResponseEntity get(@PathVariable("id") int id) {
		try {
			Optional<Linguagem> optional = linguagemService.get(id, context.getTenant());
	...
	public ResponseEntity save(@RequestBody JsonLinguagem jsonLinguagem) {
		try {
			Linguagem linguagem = Parser.toEntity(jsonLinguagem);

			linguagem = linguagemService.save(linguagem);

			return ResponseEntity.status(CREATED).body(Parser.toJson(linguagem));
			...
			return ResponseEntity.status(BAD_REQUEST).body(new JsonError(e, message, jsonLinguagem, e.getLegalMessage()));
		} catch (Exception e) {
			String message = String.format("Não foi possivel salvar  linguagem [ %s ] parametros [ %s ]", e.getMessage(), jsonLinguagem.toString());
			LOGGER.error(message, e);
			return ResponseEntity.status(INTERNAL_SERVER_ERROR).body(new JsonError(e, message, jsonLinguagem));

		}
	}

	@RequestMapping(value = "{id:\\d+}", method = PUT, produces = APPLICATION_JSON_VALUE)
	public ResponseEntity update(@PathVariable("id") Integer id, @RequestBody JsonLinguagem jsonLinguagem) {
		try {
			Linguagem linguagem = Parser.toEntity(jsonLinguagem);

			linguagem = linguagemService.update(linguagem);

			return ResponseEntity.ok(Parser.toJson(linguagem));
		} catch (ValidationException e) {
			String message = String.format("Não foi possivel salvar  o registro [ %s ] parametros [ %s ]", e.getOrigem().getMessage(), jsonLinguagem.toString());
			LOGGER.error(message, e.getOrigem());
			return ResponseEntity.status(BAD_REQUEST).body(new JsonError(e, message, jsonLinguagem, e.getLegalMessage()));
	...
	public ResponseEntity delete(@PathVariable("id") Integer id) {
		try {
			linguagemService.delete(id, context.getTenant());
